...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        corrected_q = torch.clamp(torch.round(self.q), min=1.0, max=255.0)
        x = torch.clamp(torch.round(x / corrected_q) * self.q, min=0.0, max=255.0)
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        test(args, net, device, test_loader, criterion)

    if args.save_model:
        torch.save(net.state_dict(), 'cifar_cnn.pt')
